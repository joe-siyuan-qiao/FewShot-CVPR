...
local function createModel(opt)
   ...
   elseif opt.dataset == 'miniimagenet' then
      if opt.simpleconvnet == 'false' then
         print (' | ResNet for MiniImageNet')
         cfg = {160, 320, 640}
         iChannels = cfg[1] / 2
         model:add(Convolution(3, iChannels, 3, 3, 1, 1, 1, 1))
         model:add(SBatchNorm(iChannels))
         model:add(ReLU(true))
         model:add(layer(basicblock, cfg[1], 4, 2))
         model:add(layer(basicblock, cfg[2], 4, 2))
         model:add(layer(basicblock, cfg[3], 4, 2))
         model:add(Avg(10, 10, 1, 1))
         model:add(nn.View(cfg[3]):setNumInputDims(3))
         model:add(nn.Linear(cfg[3], 80))
         print (model)
      else
         print (' | Simple ConvNet for MiniImageNet')
